  ...
  <div class="book-list">
    {% for item in collections.texts %}
      ...
      {% if "sub-intro" not in (item.data.tags or []) %}
        <div class="book-card">
          <h3><a href="{{ item.url | url }}">{{ item.data.title | replace("מבוא לחיבור ", "") | replace("מבוא לספר ", "") | replace("חזון דניאל הסורי-ארמי: הקדמה", "חזון דניאל ארמי-סורי") | replace("חזון דניאל (סורי-ארמי) – הקדמה", "חזון דניאל ארמי-סורי") | replace("דניאל הקטן (ארמית-סורית): הקדמה", "דניאל הקטן") | replace(": הקדמה", "") | replace("הקדמה", "") | replace("מבוא", "") | trim }}</a></h3>
