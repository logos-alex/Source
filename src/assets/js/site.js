...
(() => {
  ...
  function initMobileNavigation() {
    ...
    nav.querySelectorAll(".nav-dropdown").forEach((dropdown, index) => {
      ...
      toggle.setAttribute(
        "aria-label",
        `פתיחה וסגירה של תת-תפריט ${dropLink.textContent.trim()}`,
      );
      toggle.textContent = "▾";
  ...
  function extractText(el) {
    ...
    clone
      .querySelectorAll(
        "script, style, .text-controls, .parallel-controls, .version-parallel-controls, .version-parallel-panel, .reading-progress, .footnotes",
      )
      .forEach((e) => e.remove());
  ...
  function initVersionParallel() {
    ...
    let alignment = null;
    const alignmentScript = document.querySelector(
      'script[data-alignment="' + book + '"]',
    );
    if (alignmentScript) {
      try {
    ...
    if (
      savedTarget &&
      select.querySelector('option[value="' + savedTarget + '"]')
    ) {
      ...
      if (sectionLabel) {
        if (currents && currents.length) {
          ...
          sectionLabel.textContent =
            "קטע" + (names.length > 1 ? "ים" : "") + ": " + names.join(" · ");
    ...
    function applyDiffState(enabled) {
      if (diffToggle) {
        ...
        diffToggle.querySelector("span").textContent = enabled
          ? "הדגש הבדלים פעיל"
          : "הדגש הבדלים";
      }
      if (enabled) {
    ...
    function renderParallelWithDiff() {
      ...
      const localContent = document.querySelector(
    ...
    function updateStatsAndNote() {
      ...
      const pctDiff =
        totalA > 0
          ? Math.round(((removeCount + addCount) / (totalA + totalB)) * 100)
          : 0;

      // Display stats
      if (statsEl) {
        statsEl.innerHTML =
          `<span class="version-parallel-stats__label">הבדלים בקטע זה:</span> ` +
      ...
      if (noteEl) {
        ...
        if (note) {
          noteEl.innerHTML =
            '<span class="version-parallel-note__label">הערת השוואה:</span> ' +
            note;
          noteEl.hidden = false;
        } else {
          noteEl.hidden = true;
    ...
    function loadParallelContent() {
      ...
      let pages;
      if (targetSpans) {
        pages = [...new Set(targetSpans.map((s) => s.page))].sort(
          (a, b) => a - b,
        );
      } else {
      ...
      const rangeLabel =
        minPage === maxPage ? `פרק ${minPage}` : `פרקים ${minPage}–${maxPage}`;
      ...
      const urls = pages.map(
        (p) => `/Source/texts/${source}/${book}/${targetVersion}/page-${p}/`,
      );

      Promise.all(
        urls.map((u) => fetch(u).then((r) => (r.ok ? r.text() : null))),
      )
        .then((htmls) => {
          ...
          htmls.forEach((html, idx) => {
            ...
            const remoteArticle =
              doc.querySelector("article.text-main .text-content") ||
              doc.querySelector("article.text-main");
            if (!remoteArticle) return;
            remoteArticle
              .querySelectorAll(
                ".text-controls, .parallel-controls, .version-parallel-controls, .version-parallel-panel, .footnotes, .reading-progress, script",
              )
              .forEach((el) => el.remove());
            const pageNo = pages[idx];
            const spansForPage = targetSpans
              ? targetSpans.filter((s) => s.page === pageNo)
              : [];
            const hasExactSpans =
              spansForPage.length > 0 &&
              spansForPage.every(
                (s) => s.from !== null && s.to !== null && s.to >= s.from,
              );
            let sents = null;
            if (hasExactSpans) {
              const raw = remoteArticle.textContent
                .replace(/\n{2,}/g, "\n")
                .trim();
              sents = raw
                .split(/(?<=[\.\?\!\"\u05C3])\s+/)
              ...
              if (slice.hasExactSpans) {
                slice.spansForPage.forEach((sp) => {
                  if (!sectionGroups.has(sp.sectionKey)) {
                    sectionGroups.set(sp.sectionKey, {
                  ...
                  sectionGroups
                    .get(sp.sectionKey)
                    .spans.push({ span: sp, sents: slice.sents });
                });
              } else {
          ...
          if (
            diffToggle &&
            diffToggle.getAttribute("aria-pressed") === "true"
          ) {
            renderParallelWithDiff();
          }
        })
  ...
  function initParallelToggle() {
    ...
    function applyState(visible) {
      document
        .querySelectorAll("[data-parallel-container]")
        .forEach((container) => {
          if (visible) {
      ...
      toggles.forEach((btn) => {
        ...
        const isActive =
          (mode === "show" && visible) || (mode === "hide" && !visible);
...
const runtimeThirdParty =
  (window.siteRuntimeConfig && window.siteRuntimeConfig.thirdParty) || {};
...
const runtimeDisqus =
  (window.siteRuntimeConfig &&
    window.siteRuntimeConfig.thirdParty &&
    window.siteRuntimeConfig.thirdParty.disqus) ||
  {};
...
function ensureExternalScript({ service, src, onload, id }) {
  ...
  const existing = document.querySelector(
    `script[data-external-service="${service}"][src="${src}"]`,
  );
  if (existing) {
...
function loadAnalytics() {
  if (
    !runtimeAnalytics.enabled ||
    isServiceLoaded("analytics") ||
    !runtimeAnalytics.measurementId
  )
    return;
...
function loadClarity() {
  if (
    !runtimeClarity.enabled ||
    isServiceLoaded("clarity") ||
    !runtimeClarity.projectId
  )
    return;
  window.clarity =
    window.clarity ||
...
const translateLauncher = document.querySelector(
  '[data-third-party-trigger="translate"]',
);
  ...
  new google.translate.TranslateElement(
    {
      pageLanguage: "he",
      includedLanguages:
        runtimeTranslate.includedLanguages || "en,fr,de,es,ru,ar,it",
...
(function initFontControls() {
  ...
  const initialActive =
    Array.from(buttons).find((b) => b.dataset.scale === stored) || buttons[0];
