...
const sitePrefix =
  JSON.parse(readFileSync("src/_data/site.json", "utf8")).pathPrefix || "/";
const errors = [];
const htmlFiles = [];
...
const URL_ATTR_RE =
  /(?:href|src|action|poster|data-url|data-href|data-src)\s*=\s*"([^"]+)"/g;
const SRCSET_RE = /srcset\s*=\s*"([^"]+)"/g;
const META_URL_RE =
  /<meta[^>]+content\s*=\s*"((?:\/[^"]+"|https?:\/\/[^"]+))"/g;
...
function check(file) {
  ...
  while ((m = URL_ATTR_RE.exec(html))) {
    ...
    if (
      /^(https?:)?\/\//.test(url) ||
      /^(mailto:|tel:|javascript:|data:)/.test(url)
    )
      continue;
    if (url.startsWith("#")) continue;
    // Ignore false-positive Hebrew/Aramaic text misparsed as URL
    if (!url.startsWith("/") && /[\u0590-\u05FF\u0600-\u06FF\s]/.test(url))
      continue;
...
if (errors.length) {
  ...
  console.error(
    `Built-link verification failed (${errors.length} issues, showing first ${shown}):\n`,
  );
...
console.log(
  `Built-link verification passed (${htmlFiles.length} HTML files scanned).`,
);
