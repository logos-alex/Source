...
const CATALOG = JSON.parse(
  readFileSync("src/_data/sources-catalog.json", "utf8"),
);
const FIGURES = JSON.parse(readFileSync("src/_data/figures.json", "utf8"));
const FIGURE_CATALOG_KEYS = JSON.parse(
  readFileSync("src/_data/figureCatalogKeys.json", "utf8"),
);

// Load bible section keys — figures that are also section keys have their pages
// generated by bible-section.njk instead of by-figure.njk
let BIBLE_SECTION_KEYS = new Set();
try {
  const bibleSections = JSON.parse(
    readFileSync("src/_data/bibleSections.json", "utf8"),
  );
  if (bibleSections && bibleSections.sections) {
    BIBLE_SECTION_KEYS = new Set(Object.keys(bibleSections.sections));
  }
} catch (e) {
  // bible-sections.json may not exist in older versions
}
...
function check(file) {
  ...
  if (book && !KNOWN_BOOK_IDS.has(book)) {
    errors.push(
      `${file}: unknown book '${book}' (not found in sources-catalog.json)`,
    );
  }
  ...
  if (book && BOOK_BY_ID.has(book)) {
    ...
    if (entry.figure && figure && entry.figure !== figure) {
      errors.push(
        `${file}: figure mismatch — frontmatter has '${figure}' but catalog declares '${entry.figure}' for book '${book}'`,
      );
    }
  }
}
...
for (const [figure, files] of usedFigures) {
  ...
  const hasDynamicPage =
    ACTIVE_FIGURE_KEYS.has(figure) || BIBLE_SECTION_KEYS.has(figure);
