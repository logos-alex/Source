...
function toCollectionItem(file) {
  ...
  return {
    inputPath: file,
    fileSlug: file.endsWith("/index.md")
      ? "index"
...
for (const chapter of firstChapters) {
  ...
  const currentIndex = pagesInBook.findIndex(
    (item) => item.url === chapter.url,
  );
  if (currentIndex <= 0) {
    errors.push(
      `${chapter.inputPath}: first chapter is missing an intro page before it`,
    );
    continue;
  }
  ...
  if (
    Number(prevPage?.data?.pageNumber || 0) !== 0 &&
    !prevPage?.inputPath?.endsWith("/index.md")
  ) {
    errors.push(
      `${chapter.inputPath}: previous page is '${prevPage?.inputPath || "unknown"}' instead of the intro page`,
    );
    continue;
  }
  ...
  if (prevPage.inputPath !== expectedIntroPath) {
    errors.push(
      `${chapter.inputPath}: previous page resolves to '${prevPage.inputPath}' instead of '${expectedIntroPath}'`,
    );
  }
}
...
console.log(
  `Intro-navigation verification passed for ${firstChapters.length} first-chapter page(s).`,
);
