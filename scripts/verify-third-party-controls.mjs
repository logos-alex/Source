...
process.on("uncaughtException", (err) => {
  console.error(err);
  restoreConfig("uncaughtException");
});
...
try {
  ...
  assert(
    siteJs.includes("window.__externalScriptRegistry"),
    "site.js should keep a registry for deduplicating external scripts",
  );
  assert(
    siteJs.includes("data-external-service"),
    "site.js should mark injected external scripts for duplicate prevention",
  );
  assert(
    siteJs.includes("loadAnalytics"),
    "site.js should expose loadAnalytics",
  );
  ...
  if (tryBuild(disabledConfig, "disabled-config build")) {
    const disabledHtml = walkHtmlFiles(path.join(repoRoot, "_site"))
      .map((file) => readFileSync(file, "utf8"))
      .join("\n");
    assert(
      !disabledHtml.includes("translateLauncher"),
      "Translate launcher should disappear when translate is disabled",
    );
    assert(
      !disabledHtml.includes("disqus_thread"),
      "Disqus container should disappear when disqus is disabled",
    );
    assert(
      !disabledHtml.includes("googletagmanager.com/gtag/js"),
      "Analytics script should disappear when analytics is disabled",
    );
    assert(
      !disabledHtml.includes("clarity.ms/tag/"),
      "Clarity script should disappear when clarity is disabled",
    );
    assert(
      !disabledHtml.includes("fonts.googleapis.com"),
      "External font stylesheet should disappear when external fonts are disabled",
    );
  }
  ...
  if (tryBuild(enabledConfig, "enabled-config build")) {
    ...
    for (const file of htmlFiles) {
      ...
      const analyticsScripts =
        html.match(/googletagmanager\.com\/gtag\/js/g) || [];
      const clarityScripts = html.match(/clarity\.ms\/tag\//g) || [];

      if (translateLaunchers.length > 1)
        failures.push(
          `${path.relative(repoRoot, file)} injects translate launcher more than once`,
        );
      if (analyticsScripts.length > 1)
        failures.push(
          `${path.relative(repoRoot, file)} injects analytics script more than once`,
        );
      if (clarityScripts.length > 1)
        failures.push(
          `${path.relative(repoRoot, file)} injects clarity script more than once`,
        );
    }
  }
  ...
  if (buildWarnings.length) {
    console.warn(
      "Static third-party QA passed, but build-backed checks were skipped due to environment limitations:",
    );
