...
const SOURCE_KEYS = Object.keys(
  JSON.parse(readFileSync(LANGUAGES_DATA, "utf8")),
);
...
function checkPage(file, frontmatter, base) {
  ...
  if (!hasKey(frontmatter, "tags")) {
    errors.push(
      `${file}: missing required key 'tags' (needed for sitemap inclusion)`,
    );
  } else {
  ...
  if (hasKey(frontmatter, "pageNumber")) {
    ...
    if (!Number.isInteger(numeric) || numeric <= 0) {
      errors.push(
        `${file}: pageNumber must be a positive integer (got '${raw}')`,
      );
    } else {
      ...
      if (fileMatch && Number(fileMatch[1]) !== numeric) {
        errors.push(
          `${file}: pageNumber ${numeric} does not match filename page-${fileMatch[1]}`,
        );
      }
    }
  }
  ...
  if (hasKey(frontmatter, "permalink")) {
    ...
    if (/^page-\d+\.md$/.test(base) && !expected.test(permalink)) {
      errors.push(
        `${file}: permalink '${permalink}' does not match expected format /texts/<source>/<book>/[version/]page-N/`,
      );
    }
  }
