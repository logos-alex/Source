...
---
Task ID: 23
Agent: Super Z (main) — Round 4 audit: infrastructure improvements
Task: סבב רביעי — שיפורי תשתית: site.json, CSS, a11y, תיעוד, הקמת scripts/_lib/

Work Log:
- [A] קריטי — תיקון site.json (סתירת פונטים):
  · useSelfHosted: false → true, loadExternal: true → false
  · עכשיו האתר טוען פונטים מ-16 קבצי woff2/ttf ב-/assets/fonts/ במקום מ-Google CDN
  · חוסך בקשות חיצוניות, משפר פרטיות, מנצל עבודה קודמת של self-hosting
- [B] קריטי — תיקון הצהרות שקריות בדף הבית:
  · index.njk: "661 פרקי תוכן ומעל 500,000 מילים" → "29 חיבורים רשומים בקטלוג... הטקסטים עצמם נמצאים בהכנה"
  · הרשימה שמתחת עודכנה מ"השלמת חיבורים בנוסח מקוטע" ל"שילוב הטקסטים המלאים של 29 החיבורים"
- [C] CSS — מיזוג כפילויות:
  · @media (max-width: 880px) היה מוגדר פעמיים (1538 + 1956) → מוזג לבלוק אחד
  · .mobile-search-link היה מוגדר פעמיים → הועבר לתוך ה-@media המאוחד
  · נוספה הגדרת ברירת מחדל .mobile-search-link { display: none; } שהייתה חסרה
- [D] CSS — איחוד בלוקי :root:
  · --c-surface-accent הועבר מבלוק :root מקוטע בסוף הקובץ ל-:root הראשי (שורה 34) + dark mode (שורה 114)
  · הוסרו 2 בלוקי :root מקוטעים מסוף הקובץ
- [E] CSS — הסרת משתנים מתים (6 משתנים):
  · --c-bg-sunken (הוגדר ב-light + dark, לא היה בשימוש)
  · --c-border-strong (הוגדר ב-light + dark, לא היה בשימוש)
  · --c-gold-soft (הוגדר ב-light + dark, לא היה בשימוש)
  · --fs-4xl (הוגדר, לא היה בשימוש)
  · --r-xl (הוגדר, לא היה בשימוש)
  · --s-10 (הוגדר, לא היה בשימוש)
  · Pagefind vars נשמרו — מיועדים להתאמת UI עתידית
- [F] CSS — ארגון z-index לסקייל מסודר:
  · נוספו 6 משתנים ב-:root: --z-base (1), --z-dropdown (50), --z-overlay (90), --z-sticky (100), --z-toast (200), --z-skip-link (1000)
  · 6 הופעות של z-index מספרי הוחלפו במשתנים תואמים
  · הערות תיעודיות לכל רמה
- [G] a11y — הסרת inline style מ-.icon-sun:
  · base.njk: style="display:none;" הוסר מ-SVG אייקון שמש
  · CSS חדש: .icon-sun { display: none; } + html[data-theme="dark"] .icon-sun/.icon-moon — חילופי תצוגה דרך CSS בלבד
  · JS: הסרת הקוד שהחליף style.display ידנית (שורות 55-60 ב-site.js)
- [H] הקמת scripts/_lib/ — מודולים משותפים לסקריפטים עתידייering:
  · scripts/_lib/walk.mjs — walkFiles() + listFiles() (מחליף 16 הגדרות walk כפולות)
  · scripts/_lib/frontmatter.mjs — getFrontmatter(), hasKey(), readValue(), parseTags() (תומך block + inline)
  · scripts/_lib/catalog.mjs — getCatalog(), getFigures(), getLanguages(), getBookIds(), getParallelBookIds() עם caching
  · scripts/_lib/index.mjs — barrel export
  · אומת: כל הפונקציות נטענות ומחזירות נתונים נכונים (29 ספרים, 9 parallel, 8 שפות, 8 דמויות)
  · סקריפטים קיימים לא שונו (שמרני — יעברו ל-_lib בסבב עתידי)
- [I] תיעוד — שכתוב README:
  · "מצב המאגר" הוחלף ל: "29 חיבורים רשומים / 0 פרקים / 0 מילים / 29 ב-comingSoon"
  · נוסף פירוט לפי שפה (8 שפות, 29 חיבורים)
  · נוסף פסקה "מה מוכן עכשיו" (תשתית טכנית מלאה)
  · נוסף פסקה "מה חסר" (placeholder + הוראות הפעלה)
  · נוסף תיעוד scripts/_lib/
  · נוספה הערה: "ה-CI בודק עקביות מבנית בלבד, לא נוכחות תוכן"

Stage Summary:
- **2 קריטיים תוקנו:** self-hosted fonts פעיל, הצהרות שקריות הוסרו מדף הבית
- **CSS נוקה:** 6 כפילויות/מתים הוסרו, z-index scale מסודר, :root מאוחד
- **a11y שופר:** inline style הוסר, חילופי תצוגה דרך CSS בלבד
- **תשתית עתידית:** scripts/_lib/ מוכן לשימוש בסקריפטים חדשים
- **תיעוד מדויק:** README משקף מצב אמיתי (תשתית בלבד, 0 פרקים)
- סה"כ 4 סבבים: 59 בעיות תוקנו, 0 רגרסיות (CI עדיין עובר 16/16)

---
Task ID: 24
Agent: Super Z (main) — שילוב חזון אברהם (3 נוסחים)
Task: שילוב מלא של חזון אברהם — 3 נוסחים עבריים עצמאיים מתוך קובץ RAR שהעלה המשתמש

Work Log:
- [A] חילוץ RAR (522KB) ל-3 קבצי markdown ראשיים + חומרי מקור:
  · "חזון אברהם א.md" — נוסח א' (שחזור פילולוגי), 23 פרקים + נספח
  · "חזון-אברהם-נוסח-ב.md" — נוסח ב' (מילולי מלא), 24 פרקים + נספח
  · "חזון-אברהם-נוסח-ג-פורפיריייב-שחזור-עברי.md" — נוסח ג' (פורפירייב), 28 פרקים + נספח
- [B] עדכון sources-catalog.json:
  · הוספת נוסח ג' (פורפירייב) כ-version נוסף
  · עדכון תיאורי a/b/c עם מאפיינים מדויקים (רגיסטר, מספר פרקים, נספח)
  · comingSoon: true → false
- [C] כתיבת סקריפט Python (scripts/split_apocalypse_abraham.py):
  · strip_front_matter: הפרדת H1 + italic intro מ-גוף הטקסט על ידי `---` separator
  · split_chapters: חלוקה לפי `## ` (בלי `###`) עם שמירת תת־סעיפים בתוך הפרק
  · is_appendix: זיהוי נספח (כותרת שאינה אות עברית בודדת)
  · chapter_title: "פרק <letter>" לפרקים, "נספח: <full heading>" לנספחים
  · chapter_description: תיאור meta קצר לכל עמוד
  · make_frontmatter: פורמט תקין עם layout, title, book, source, figure, pageNumber, version, permalink, tags, description
- [D] יצירת 78 קבצי page-N.md:
  · a/: page-1 עד page-24 (23 פרקים + נספח "צמתים טעוני דיון")
  · b/: page-1 עד page-25 (24 פרקים + נספח "הבדלי נוסח מול נוסח א")
  · c/: page-1 עד page-29 (28 פרקים + נספח "הבדלי נוסח מול נוסח א ו-ב")
- [E] כתיבת 4 מבואות:
  · index.md ראשי — מבוא מלא (~600 מילה): מקור, מבנה, 3 נוסחים, מעמד במסורת, עקרונות, מראי מקום
  · a/index.md — מבוא לנוסח א' עם עקרונות השחזור, מבנה, הערה מתודולוגית, השוואה
  · b/index.md — מבוא לנוסח ב' עם תיאור תוספת פרק כ"ד (סיום מפורש), מבנה, השוואה
  · c/index.md — מבוא לנוסח ג' עם תיאור חומר ייחודי ("הצהרת־אל השנייה" + סיום אמיתי), מבנה, השוואה
- [F] תיקון קישורים פנימיים — מעבר מ-markdown links ל-Nunjucks syntax:
  · 12 קישורים בין index.md ל-versions תוקנו מ-`[text](/path/)` ל-`[text]({{ '/path/' | url }})`
  · סיבה: קישורי markdown לא עוברים pathPrefix (/Source/) — verify-path-prefix תפס 12 קישורים
- [G] עדכון docs/content-status-he.md — נוצר מחדש על ידי npm run report:content
  · 78 פרקים, 12,536 מילים, 28 ספרים ב-comingSoon (היה 29)
- [H] תוצאות build:
  · 136 עמודי HTML (מ-57 קודם — 79 חדשים)
  · Pagefind אינדקס מלא
  · by-figure/abraham/ נוצר אוטומטית מתבנית by-figure.njk (4 כרטיסים: main + a + b + c)
  · כל 16 בדיקות CI עוברות (13.25s)

Stage Summary:
- **חזון אברהם שולב במלואו — 3 נוסחים, 78 פרקים, 12,536 מילים**
- **נוסח א':** 23 פרקים + נספח "צמתים טעוני דיון" — שחזור פילולוגי מן הסלאבית
- **נוסח ב':** 24 פרקים + נספח "הבדלי נוסח מול נוסח א" — שחזור מילולי מלא עם סיום מפורש
- **נוסח ג':** 28 פרקים + נספח "הבדלי נוסח מול נוסח א ו-ב" — שחזור פורפירייב עם חומר ייחודי
